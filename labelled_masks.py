...
ROOT_DIR = "Data"
OUTPUT_DIR = os.path.join(ROOT_DIR, "test_labelled")
...
def clean_filename(filename):
    ...
    """
    base = filename.split("_jpg.")[0]  # remove _jpg. part
    base = os.path.splitext(base)[0]  # remove extension
    return base


def process_dataset(dataset_path, class_value, prompt_suffix):
...
# RUN
process_dataset(
    os.path.join(ROOT_DIR, "cracks/test"),
    CRACK_LABEL,
    "segment_crack"
)

process_dataset(
    os.path.join(ROOT_DIR, "tapping/test"),
    TAPING_LABEL,
    "segment_taping_area"
)
